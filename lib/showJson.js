...
const updateDecorations = currentEditor => {
    ...
    const modulePrefixFoUpdateJson = settings.get('modulePrefixFoUpdateJson');
    ...
    fs.readFile(localesPath, (err, data) => {
        if (!err) {
            const i18nObj = !!modulePrefixFoUpdateJson
                ? JSON.parse(data.toString())[modulePrefixFoUpdateJson]
